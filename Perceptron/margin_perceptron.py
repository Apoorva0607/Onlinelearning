# -*- coding: utf-8 -*-
"""
Created on Sun Sep 24 14:48:29 2017
...
def train(epochs,r_2,w,newX,ct,mu):
    ...
    for j in range(epochs):
        for i in range(0,len(newX)):
            if (newX[i][-1]*(np.dot(w,newX[i][0:-1])))<mu:
               w[0:-1]=w[0:-1]+r_2*newX[i][-1]*newX[i][0:-2]
               w[-1]=w[-1]+r*newX[i][-1]
...
mu=0.1   ### Taken from finding hyperparameters
length=X.shape[1] 
...
ct=0
w1,ct=train(epochs,r,w,newX,ct,mu)
...
ct=0
w_1,acc_deV=traindev(epochs,r,w,newX,ct,newX2,labels2,mu)    
